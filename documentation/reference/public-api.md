...
| `tamoz-approval` | The approval/permission policy owner: immutable `Request`/`Decision`/`Grant`/`GrantOffer` values, the error taxonomy, `Answer.parse`, `PolicyDocument`, and the `Engine` with its `GrantStore`/`DecisionLog` ports |
