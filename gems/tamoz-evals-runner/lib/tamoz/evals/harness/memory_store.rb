  ...
  module Evals
    module Harness
      ...
      class MemoryStore
        ...
        def self.seed(path, seed_records)
          store = new(path)
        ...
        def seed(seed_records)
          ...
          write_stable(INDEX_FILE, {"records" => seed_records.map { |record| index_entry(record) }})
          write_stable(VAULT_FILE, {"records" => restricted_bodies(seed_records)})
          @seed_digest = digest
          @seed_digest
        end
        ...
        def restricted_bodies(seed_records)
          seed_records.each_with_object({}) do |record, bodies|
