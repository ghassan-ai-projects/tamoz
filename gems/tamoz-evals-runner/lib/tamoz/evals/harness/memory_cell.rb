  ...
  module Evals
    module Harness
      ...
      class MemoryCell
        ...
        # P11-ED: a `store_factory` (callable(cell_root, seed_records) -> store)
        # builds a per-cell store over the REAL production memory stack
        # (`MemoryRepositoryAdapter`); the default seeds the DR-3 JSON store
        # store. Both keep per-cell isolation + the seed-digest precondition.
        def seed_store
          seed_records = @case_artifact.to_h.fetch("treatments", {}).dig("seed", "fixtures")
          if @store_factory
            @store_factory.call(@cell_root, seed_records)
          else
            path = File.join(@cell_root, "store", "store.json")
            MemoryStore.seed(path, seed_records)
          end
        end
