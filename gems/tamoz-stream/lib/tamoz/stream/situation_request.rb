  ...
  module Stream
    ...
    class EpisodeRequestEnvelope
      ...
      def budget_hash
        ...
        {
          "max_model_calls" => positive_int_or_nil(budget.max_model_calls),
          "max_input_tokens" => positive_int_or_nil(budget.max_input_tokens),
          "max_output_tokens" => positive_int_or_nil(budget.max_output_tokens),
          "max_tool_calls" => positive_int_or_nil(budget.max_tool_calls),
          "max_tool_result_bytes" => positive_int_or_nil(budget.max_tool_result_bytes),
          "max_total_tool_result_bytes" => positive_int_or_nil(budget.max_total_tool_result_bytes),
          "max_provider_retries" => positive_int_or_nil(budget.max_provider_retries),
          "max_cost_microunits" => positive_int_or_nil(budget.max_cost_microunits)
        }
      end
      ...
      def watch_confidence_floor
        if @wire.has_watch_confidence_floor?
      ...
      def validate!
        ...
        validate_fence!
        validate_watch_confidence_floor!
      ...
      def validate_protocol_version!
        return if @wire.protocol_version == PROTOCOL_VERSION
      ...
      def validate_identity_field!(field)
      ...
      def validate_fence!
        return if @wire.fence.is_a?(Integer) && @wire.fence >= 1

        raise EpisodeRequestInvalidError,
              "episode fence must be a positive integer"
      end

      def validate_watch_confidence_floor!
        floor = watch_confidence_floor
        return if floor.is_a?(Numeric) && floor.finite? && floor >= 0

        raise EpisodeRequestInvalidError,
              "watch_confidence_floor must be finite and non-negative"
      end

      def validate_allowed_intent_types!
        return if @wire.allowed_intent_types.to_a.length <= 16

        raise EpisodeRequestInvalidError,
              "allowed_intent_types must not exceed 16 entries"
      end

      def validate_intent_catalog_for_kind!
        return unless diagnose? || reconsider?
        return unless @wire.intent_catalog_json.to_s.empty? || intent_catalog_sha256.to_s.empty?

        raise EpisodeRequestInvalidError,
              "a #{kind} episode requires the intent catalog"
      end

      def validate_kind!
        return if KIND_NAMES.key?(@wire.kind)
      ...
      def validate_lane!
        return if LANE_NAMES.key?(@wire.lane)

        raise EpisodeRequestInvalidError,
              "unsupported episode lane #{@wire.lane.inspect}"
      end

      def validate_risk_ceiling!
        return if RISK_NAMES.key?(@wire.risk_ceiling)
    ...
    class EpisodeRunner
      ...
      def run(wire_request, call = nil)
        Enumerator.new do |yielder|
          ...
          watcher = nil
          begin
            envelope, snapshot = admit_request(wire_request)
      ...
      def fail_stream(stream, wire_request, error, code:)
      ...
      def admit_request(wire_request)
        ...
        verify_snapshot_identity!(wire_request, snapshot)
        verify_configured_tenant!(wire_request, snapshot) if @configured_tenant
        [envelope, snapshot]
      end

      def verify_snapshot_identity!(wire_request, snapshot)
      ...
      def verify_configured_tenant!(wire_request, snapshot)
        if wire_request.tenant_id != @configured_tenant ||
      ...
      def load_terminal_state(envelope, result)
        ...
        checkpoint.state.to_h
      end

      def emit_budget_update(stream, adapter, terminal_state)
      ...
      # checkpoint is the request's OWN terminal checkpoint (never the thread's
      # `latest`, which a concurrent fence+1 redispatch could have moved past).
      ...
      def build_artifact_manifest(envelope, terminal_state = nil)
        terminal_state ||= {}
        validate_terminal_memory_consistency!(terminal_state)

        Agenticstream::Runtime::V1::ArtifactManifest.new(
          prompt_sha256: digest_bytes_or_nil(envelope.prompt_sha256),
          skill_set_sha256: digest_bytes_or_nil(terminal_state[:skill_set_digest]),
          ...
          memory_record_sha256: Array(terminal_state[:memory_record_digests]).map do |digest|
            Tamoz::Core.digest_bytes(digest)
          end
        )
      end

      def validate_terminal_memory_consistency!(terminal_state)
        return unless terminal_state.key?(:memory_record_digests) || terminal_state.key?(:situation_memory)
      ...
      def retain_manifest_artifacts(envelope, terminal_state = nil)
        retain_manifest_documents!(envelope)
        retain_terminal_raw_responses!(terminal_state)
      end

      def retain_manifest_documents!(envelope)
        manifest_documents(envelope).each do |name, (wire_digest, bytes)|
          next if bytes.to_s.empty?

          retain_manifest_document!(name, wire_digest, bytes.to_s)
        end
      end

      def manifest_documents(envelope)
        {
      ...
      def retain_manifest_document!(name, wire_digest, bytes)
        verify_manifest_digest!(name, wire_digest, bytes)
      ...
      def retain_terminal_raw_responses!(terminal_state)
      ...
      def build_capability_host(wire_request, snapshot)
        source = evidence_source_for(wire_request, snapshot)
      ...
      def build_tool_implementations(source)
        EpisodeCapabilityHost::PERMITTED.to_h do |name|
          adapter = if source == :unavailable
                      EvidenceUnavailableAdapter.new(tool_name: name)
                    else
                      EvidenceToolAdapter.new(source, tool_name: name)
                    end
          [name, adapter]
        end
      end
