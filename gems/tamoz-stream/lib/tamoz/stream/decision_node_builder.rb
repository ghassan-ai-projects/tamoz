...
module Tamoz
  module Stream
    ...
    # tamoz-stream — it owns the frozen decision-v1 schema, the allowlist
    # logic, and (until P4 replaces them) the ACTION_RISKS table. The node
    # passes the validated document projection and the episode view; the
    # builder returns the [decision, digest] pair the terminal graph state
    # carries. The runner only translates that state to the wire.
    class DecisionNodeBuilder
      def call(document:, episode:, snapshot:, snapshot_digest:, allowlist:)
        outcome = {
