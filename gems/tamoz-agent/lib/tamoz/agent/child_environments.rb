...
module Tamoz
  module Agent
    ...
    # key, the worker gets the model key and ALMS endpoint but never the bot
    # token, queue/status get neither, and the harness gets only sanitized
    # pointers. A value that is not in the map never reaches the child —
    # there is no shared-env path.
    module ChildEnvironments
      ...
      WORKER_ONLY = %w[TAMOZ_ALMS_MCP_ENDPOINT].freeze
      ...
      def self.worker_env(base, runtime_dir:)
        ...
        standard_env(base).merge(
          ...
          'TAMOZ_ALMS_MCP_ENDPOINT' => base.fetch('TAMOZ_ALMS_MCP_ENDPOINT'),
