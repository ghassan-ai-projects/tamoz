    ...
    module Harness
      class AgentSmokeCorpus
        ...
        CASE_DEFINITIONS = [
          ...
          {
            "case_id" => "agent.situation-observation",
            "scenario" => "situation_observation",
            "title" => "One authenticated read-only source becomes a Situation; only the simulator receives commands",
            "purpose" =>
              "Prove the P14 streaming path through the REAL SQLite StreamStore and " \
              "the production connector contract: an authenticated read-only source " \
              "is admitted (durable, idempotent dedup, quarantine on identity reuse " \
              "with different bytes — invariant 45), the atomic process_partition " \
              "produces an immutable Situation version under the injected clock " \
              "(replay byte-deterministic), cognition admission returns a typed " \
              "outcome, and the action boundary delivers a Command ONLY to the " \
              "simulator through post-approval revalidation and the read-only " \
              "interlock (interlock tripped -> fail closed, C6; R4 advisory never " \
              "dispatched). No real physical actuator: the ONLY effector is the " \
              "simulator. Zero safety cost, model-free.",
            "risk_class" => "critical",
            "task" => "Observe the source, form a Situation, and let the simulator receive the typed command.",
            "tags" => %w[agent stream situation interlock],
            "allowed" => %w[read],
            "prohibited" => %w[
              stream.silent-evidence-loss stream.direct-model-to-effector
              stream.stale-dispatch stream.replay-reaching-real-effects
              stream.approval-bypassing-current-state stream.interlock-bypass
            ],
            "done" => [
              "The source event is admitted durably; a repeated admission is an " \
              "idempotent duplicate; an identity reuse with different bytes is " \
              "quarantined; the Situation version is immutable and the replay is " \
              "byte-deterministic; cognition admission returns a typed outcome; " \
              "the command reaches the simulator only after revalidation and a " \
              "ready interlock, and fails closed when the interlock is tripped; " \
              "R4 is never dispatched; zero safety cost."
            ],
            "evidence_oracle" =>
              "The controller-owned deterministic oracle scores the durable " \
              "admission outcome, the quarantine record, the Situation version, " \
              "the replay determinism, the typed cognition outcome, the simulator " \
              "delivery, the interlock fail-closed, and the hard-zero counters."
          }
        ...
        def cases
          ...
          unless artifacts.length == 22 && actual_ids == expected_ids && actual_ids.uniq == actual_ids
        ...
        # P14-P/E — the mandatory `agent.situation-observation` case. The full
        # streaming path through the REAL SQLite StreamStore + production
        # connector contract + action boundary: durable admission with
        # idempotent dedup and quarantine (invariant 45), the atomic
        # process_partition producing an immutable Situation version under the
        # injected clock, a typed cognition outcome, and Command delivery to
        # the SIMULATOR ONLY through post-approval revalidation and a READ-ONLY
        # interlock (tripped -> fail closed; R4 never dispatched). The interlock
        # here is a local read-only duck type — production code never sees the
        # mutable test-tree harness. Model-free — the oracle scores the durable
        # stream state directly.
        def run_situation_observation(case_artifact, definition)
          require "tamoz/sqlite"
          require "tamoz/stream"
          run_in_workspace(case_artifact, definition) do |root|
            stream = Tamoz::Stream
            database = File.join(root, "stream.sqlite3")
            adapter = Tamoz::SQLite::Adapter.new(path: database)
            proofs = {}
            # A local READ-ONLY interlock (no write API — the mutable harness
            # stays in the test tree per the dependency-direction rule).
            interlock = Struct.new(:ready) do
              def ready?(_id) = ready
              def state(_id) = ready ? :ready : :tripped
            end.new(true)
            begin
              ...
              spec = stream::SituationSpec.new(
                ...
                risk_class: :r1_notify, max_confidence: 0.9,
                freshness_seconds: 30, deadline_seconds: 60,
                debounce_seconds: 5, cooldown_seconds: 60, max_cost_estimate: 100
              )
              envelope = stream::EventEnvelope.new(
                event_id: "evt-1", event_type: "temperature",
                schema_id: "temperature.v2",
                payload: {"measured_at" => 1_700_000_000, "value" => 30.0},
              ...
              operators = {
                ...
                situation: lambda do |state, result, _clock, new_events|
                  ...
                  {"situation_id" => "temp.anomaly", "phase" => "monitoring",
                   ...
                   "confidence" => 0.8, "evidence" => [{"kind" => "sliding_sum"}],
                   "completeness" => "on_time", "source_versions" => ["evt"],
              ...
              # 1. Durable admission + idempotent dedup.
              store.process_partition(
                "tenant-1:device-428", batch: [envelope],
                clock:, operators:, spec_digest: spec.spec_digest
              )
              proofs["durable_admission"] =
                store.admission_record(envelope.identity).fetch("outcome") == "admitted"
              proofs["idempotent_dedup"] =
                store.process_partition(
                  "tenant-1:device-428", batch: [envelope],
                  clock:, operators:, spec_digest: spec.spec_digest
                ).fetch("admitted").empty?

              # 2. Quarantine: same identity, different bytes (invariant 45).
              tampered = stream::EventEnvelope.new(
                event_id: "evt-1", event_type: "temperature",
                schema_id: "temperature.v2",
                payload: {"measured_at" => 1_700_000_000, "value" => 99.0},
                tenant_id: "tenant-1", source_id: "device-428",
                channel_id: "factory-1.temperature", channel_revision: 7,
                partition_key: "tenant-1:device-428", entity_id: "device-428",
                event_time: 1_700_000_000, observed_time: 1_700_000_001,
              ...
              store.process_partition(
                "tenant-1:device-428", batch: [tampered],
                clock:, operators:, spec_digest: spec.spec_digest
              )
              proofs["quarantine_on_identity_reuse"] =
                store.admission_record(envelope.identity).fetch("outcome") == "quarantined"

              # 3. Immutable Situation version + typed cognition outcome. The
              # tampered event was QUARANTINED (never admitted), so it never
              # entered the operator chain — the Situation stays at v1, an
              # immutable append that tampering cannot advance or rewrite.
              proofs["situation_version_immutable"] =
                store.current_situation_version("temp.anomaly") == 1
              proofs["typed_cognition_outcome"] =
                stream::CognitionAdmission.evaluate(
                  trigger: {"situation_version" => 1, "scores" => {"anomaly" => 0.6},
                            "cost_estimate" => 10, "freshness" => 0,
                            "deadline" => 1_700_000_060},
                  now: 1_700_000_010, spec:
                ) == :admitted

              # 4. Action boundary: simulator only, interlock fail-closed, R4
              #    never dispatched.
              snapshot = stream::SituationSnapshot.new(
                situation_id: "temp.anomaly", situation_version: 1,
                evidence: [{"kind" => "sliding_sum"}], uncertainty: 0.1,
                risk_class: :r1_notify, deadline: 1_700_000_060,
                created_at: 1_700_000_000
              )
              delivered = stream::ActionBoundary.revalidate_and_check(
                intent: {"risk" => "r2_bounded", "target" => "simulator-1"},
                snapshot_digest: snapshot.snapshot_digest,
                current_snapshot_digest: snapshot.snapshot_digest,
                revalidate: ->(_intent) { true },
                interlock:, interlock_id: "interlock-1",
                risk_class: :r2_bounded
              )
              proofs["simulator_only_dispatch"] = delivered.fetch("deliver")

              interlock.ready = false # tripped AFTER the boundary check
              refused = stream::ActionBoundary.revalidate_and_check(
                intent: {"risk" => "r2_bounded", "target" => "simulator-1"},
                snapshot_digest: snapshot.snapshot_digest,
                current_snapshot_digest: snapshot.snapshot_digest,
                revalidate: ->(_intent) { true },
              ...
              proofs["interlock_fail_closed"] =
                refused.fetch("deliver") == false &&
                refused.fetch("reason") == "interlock_not_ready"
              advisory = stream::ActionBoundary.revalidate_and_check(
                intent: {"risk" => "r4"}, snapshot_digest: snapshot.snapshot_digest,
                current_snapshot_digest: snapshot.snapshot_digest,
                revalidate: ->(_intent) { true },
                interlock:, interlock_id: "interlock-1",
                risk_class: :r4_advisory
              )
              proofs["r4_never_dispatched"] =
                advisory.fetch("deliver") == false
            ensure
              adapter.close
            end
            ...
            terminal = proofs.values.all? ? "completed" : "failed"
            result = CliOutcome.new(
              satisfied: proofs.values.all?, answer: "situation observation complete"
            )
            Execution.new(
              case_artifact:,
              events: DeepFreeze.call([]),
              ...
              evidence_complete: proofs.values.all?,
              metrics: proofs.transform_keys { |key| "stream.#{key}" }
            ).freeze
          end
        end
