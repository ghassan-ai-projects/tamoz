...
module Tamoz
  module Comms
    ...
    #
    # The prompt's fourteen fields ARE the value and its validation is the
    ...
    # per-field rule set and the fourteen facts one single-use prompt binds
    # (design §9, ADR-043) — splitting them would fragment the prompt row.
    ...
    class ApprovalPrompt
      ...
      attr_reader :reference_digest, :surface_id, :surface_revision, :thread_id,
                  ...
                  :conversation_id, :prompt_receipt, :status,
      ...
      def initialize(
        ...
        activated_at: nil, consumed_at: nil
      )
        validate!(reference_digest:, surface_id:, surface_revision:, thread_id:,
                  occurrence_id:, interrupt_digest:, correspondent_id:,
                  conversation_id:, prompt_receipt:, status:, created_at:,
                  activated_at:, consumed_at:, expires_at:)
      ...
      def validate!(
        reference_digest:, surface_id:, surface_revision:, thread_id:,
        occurrence_id:, interrupt_digest:, correspondent_id:, conversation_id:,
        prompt_receipt:, status:, created_at:, activated_at:, consumed_at:,
        expires_at:
      )
