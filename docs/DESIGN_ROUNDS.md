...
|---|---|---|---|---|
| DR-1 | BehaviorTransition + behavior/cache epoch (shared promotion machinery) | P11 plan review C3; P12 plan review C4/C5; checkpoint review DC-1 | P11-W, P12-I, P16 | **rev4 accepted** — allocator/active/pending split, singleton pending id, two-phase apply/finalize, bounded immutable snapshots |
| DR-2 | Durable circuit record (one type, four scopes) | P12 plan review C1; P13 plan review C6; checkpoint review DC-2 | P10 supervisor (in flight), P12-H3, P13-E, P17 | **rev3 accepted** — one aggregate scope record with bounded stable-owner sub-state, atomic CAS predicate, reset(evidence:), CircuitStore seam |
...
| DR-4 | Stale durable-request framework fix (D-6 + `:retry`) | gauntlet ledger §5.2/§5.7; checkpoint review DC-7 | P7/P8/P9 runtime, P15-B | **rev3 accepted** — pure claim-time validator, checkpointer-owned transactional failure, StaleRequestError backstop, recover-path coverage |
...
Single-active-phase order after P10: DR-4 → DR-5 → P16 → P17 → P11 → P12 →
P13 → P14 → P18 → P15. This order is authoritative for scorecard baselines and
