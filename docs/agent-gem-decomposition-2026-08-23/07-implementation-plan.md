...
| # | Move | Detail |
|---|------|--------|
| D | `Plan.parse_object/string/strings` → core | Home in `tamoz-core`; raises a core-level error (`< Tamoz::Error` — no `Core::Error` exists; `ProtocolError` is now a *sibling* of `Agent::Error`, so the CLI fatal-error ladder and the evals smoke-corpus classifier name it explicitly, P16-style). Same commit: switch the ~12 `ProtocolError` rescue sites (errors.rb def, session_plan_attempt:76,197, episode_nodes:449, session_planning_context:127, session_adaptive:84, episode_model_transport:160, witness_gateway:155,206, session_routing:35,165, runtime:176,491). DEFERRED to P3bB: folding profile.rb's private `deep_freeze` variant onto core (intent_catalog's was folded in the P0 review-fix pass; 4 pre-existing variants elsewhere remain out of scope: mcp_capability_source.rb:247, comms surface_descriptor.rb:261, tamoz-mcp canonical_json.rb:23, sqlite boundary_source_audit.rb:594). |
