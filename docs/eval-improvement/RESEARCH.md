...
## 1. Four evaluation surfaces, four jobs
...
The first two are the *capability* surfaces. `agenteval` is one pack of one of them. This
folder previously planned only `agenteval`, which is how the agent came to look like a coding
agent in its own tracking docs.
