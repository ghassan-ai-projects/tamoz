...
- **A rejected decision is terminal**: rejection → `RecordRejection` + attempt
  `failed` + episode `concluded` (agentic-stream `internal/episodes/executor.go`; drifted under the
ISSUE-061 rebind fix — `RecordRejection` call ~:462, the `concluded` UPDATE ~:497 at HEAD). No retry/re-plan with
