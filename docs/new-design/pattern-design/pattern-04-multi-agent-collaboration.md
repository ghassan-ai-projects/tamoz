...
(SubgraphRuntime, Send, ForkExecutor) is wired but dead.** Builds on: README
substrate (two-identity split, digest discipline). **Autonomy: 2-party L2-L3; the
machinery for L4 (hierarchy/debate) exists but is dead.**
...
- `Tamoz::Send` (send.rb:4-19): zero call sites. `Command` update/goto only (M2
...
| M6 | Subgraph/Send/Fork machinery dead | Wire `SubgraphRuntime` or delete it; add `max_subgraph_depth` to Limits |
