...
| 4 | 2026-08-23 | `bundle exec ruby -Itest test/comms_authority_evidence_test.rb` (6 green); comms gateway/evidence suites re-run green; `public_api_test.rb` green with `AuthorityEvidence.members` pin | `AuthorityEvidence.members = LEVELS.map(&:to_sym).freeze`; pin added to all three surfaces. Additive only — consumer lands in phase 6 boot wiring | Plumbing |
