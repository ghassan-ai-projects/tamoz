...
## F. File context (phase 2: FC1–FC8, FC10)
...
| F4 | The gate pins `expected_sha256` from the observation ledger, never from the disk at gate time; the model's own digest is ignored on the work route. | code review + G-14 |
...
| F8 | A runtime snapshot is appended only when its rendered text changes; an unchanged turn appends nothing; an emptied snapshot appends the "no longer apply" marker and never deletes; no snapshot key reaches the header. | `test/harness_runtime_snapshot_test.rb` (G-22) |
| F9 | A change ledger records `{path, before-ref, after-sha, step}`; the finish report, handoff note and post-compaction surface carry a diffstat independent of the summary text; a crash after the write does not record the post-image as the pre-image. | `test/work_loop_durability_test.rb` (G-18, G-20) |
| F10 | A `@path` reference resolves to a manifest entry with no file bytes; an outside-workspace or symlink-escaping path gains nothing; guidance files carry per-file digests and a change/removal notice. | `test/harness_file_references_test.rb` (G-13), `test/harness_instructions_test.rb` |
...
| F13 | Series-boundary normalization folds or shadows `system_update` nodes with one logged replacement; the header node is never shadowed otherwise. | `test/work_series_boundary_test.rb` (G-23) |
...
| F15 | The two phase-2 arms are registered and validated: `filectx` and `filectx-stale` appear in `HARNESS_ARMS`, `agenteval validate` accepts their selection, and `rake agenteval:harness:all` refuses an empty balance before spending a token. | `Rakefile` diff, `agenteval validate` output |
...
| F18 | Usage accounting handles both provider shapes: a route that reports only `inputTokens`/`cacheReadTokens`/`outputTokens` yields `prompt_tokens = input_uncached + cache_read`, and `totalTokens` is never used as the prompt size. | `test/context_usage_test.rb` (G-26) |
...
| D1 | met — `rake agenteval:prove` validates 38 scenarios. |
...
| F1–F18 | **not met** — phase 2 starts here. Round-by-round state in [STATUS.md](STATUS.md); every row needs its red-at-parent proof. |
