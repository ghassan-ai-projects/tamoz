...
| F2 | A `read_file` result is never spilled; an unranged work-loop read returns the policy window and a continuation footer; the 50 KiB line-boundary cap (`read.max_bytes`) is enforced by `ReadOperations` and the pipeline's `read_file` behaviour is unchanged. | `test/work_loop_observation_test.rb` (G-15) |
| F3 | An `apply_patch` the model did not read is refused `not_observed` before approval; a file changed since the read fails `stale_file` atomically; a second patch after a first needs no re-read. A file the model **created** is not an observation either. | `test/work_loop_observation_test.rb` (G-14) |
...
| F1–F4, F7 | **met, with F3/F4 qualified.** F1 in R1/R2 (`test/model_windows_test.rb`), F7 in R7 (`tools_coding_surface_test.rb`), F2 in R9 (the byte budget) and R12 (the window, the footer, never spilled), F3 and F4 in R12 (`test/work_loop_observation_test.rb`: G-14 and G-27). Not trivially passable — three of the nine cases need a working apply path — but §6.2(c)'s **positive loop-level cell is still owed** (F14, FC8), so F3/F4 are met for the refusals and the pinning, not yet for the whole row. |
