...
| # | Property | Check | Status |
|---|---|---|---|
| C1 | A deterministic fixture MCP server (test-only) serves the corpus data | eval test | PASS — `script/investigation_fixture_server` (seeded, opaque pond ids, logs every read); the oracle fails when the server returns nothing (mutation), and reads equal dispatched calls |
