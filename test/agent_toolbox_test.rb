  ...
  def test_search_text_matches_a_multibyte_query_without_a_utf8_locale
    Dir.mktmpdir("tamoz-toolbox") do |root|
      ...
      stdout, stderr, status = Open3.capture3(
        ...
        "-I#{GEM_ROOTS.fetch("tamoz-approval").join("lib")}",
        "-I#{GEM_ROOTS.fetch("tamoz-agent-kernel").join("lib")}",
        "-I#{GEM_ROOTS.fetch("tamoz-agent-memory").join("lib")}",
        "-I#{GEM_ROOTS.fetch("tamoz-agent-healing").join("lib")}",
        "-I#{GEM_ROOTS.fetch("tamoz-agent-profile").join("lib")}",
        "-I#{GEM_ROOTS.fetch("tamoz-agent-improvement").join("lib")}",
