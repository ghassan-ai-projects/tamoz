...
class EvalsVerifierTest < Minitest::Test
  ...
  def test_all_sixteen_agent_smoke_cases_are_public_verified_and_digest_pinned
    cases = AGENT_SMOKE_ROOT.glob("*.case.json").sort

    assert_equal 16, cases.length
    assert_equal 16, cases.map { |path| Tamoz::Evals::Case.load(path).digest }.uniq.length
    cases.each do |path|
      artifact = Tamoz::Evals::Case.load(path)
