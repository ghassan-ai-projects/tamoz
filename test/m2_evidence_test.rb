...
class M2EvidenceTest < Minitest::Test
  def test_fixed_runner_emits_six_verified_results
    ...
    Dir.mktmpdir("tamoz-m2-evidence") do |directory|
      stdout, stderr, status = Open3.capture3(
        {"TAMOZ_ALLOW_DIRTY_EVIDENCE" => "1"},
        RbConfig.ruby,
        ROOT.join("script", "run_m2_conformance").to_s,
        directory
      )
      ...
      expected_behavior = summary.fetch("dirty") ? "m2.graph.dirty-test" : "m2.graph"
      ...
      results.each do |path|
        ...
        assert_equal "tamoz-graph", document.dig("subject", "id")
