...
class ToolboxStagingReaperTest < Minitest::Test
  ...
  def test_a_stale_orphan_is_removed_at_action_capable_construction
    with_workspace do |root|
      orphan = plant(root, ".tamoz-abc123.tmp", mtime: ANCIENT)
      create_orphan = plant(root, ".tamoz-create-def456.tmp", mtime: ANCIENT)

      toolbox = Tamoz::Tools::Toolbox.new(root:, allow_changes: true)

      refute_path_exists orphan
      refute_path_exists create_orphan
      assert_equal [".tamoz-abc123.tmp", ".tamoz-create-def456.tmp"],
  ...
  def test_orphans_are_swept_from_nested_directories
    with_workspace do |root|
      ...
      nested = plant(root, File.join("lib", "deep", ".tamoz-nested.tmp"), mtime: ANCIENT)
  ...
  def test_a_fresh_staging_file_is_left_alone
    with_workspace do |root|
      fresh = plant(root, ".tamoz-inflight.tmp", mtime: Time.now)
  ...
  def test_the_sweep_refuses_everything_that_is_not_its_own_staging_file
    with_workspace do |root|
      ...
      near_miss = plant(root, ".tamoz-abc123.tmp.bak", mtime: ANCIENT)
      prefix_only = plant(root, ".tamozzz.tmp", mtime: ANCIENT)
      directory = File.join(root, ".tamoz-dir.tmp")
      FileUtils.mkdir_p(directory)

      # A symlink WEARING the staging name, pointing at something precious. The
      # sweep must neither follow it nor remove it.
      secret = File.join(root, "secret.txt")
      File.write(secret, "keep me\n")
      link = File.join(root, ".tamoz-evil.tmp")
      ...
      assert_empty toolbox.reaped_staging
      assert_path_exists user_file
      assert_path_exists near_miss
      assert_path_exists prefix_only
  ...
  def test_ignored_directories_are_not_traversed
    with_workspace do |root|
      %w[.git vendor node_modules].each do |ignored|
        ...
        plant(root, File.join(ignored, ".tamoz-vendored.tmp"), mtime: ANCIENT)
      end
      ...
      assert_empty toolbox.reaped_staging
      %w[.git vendor node_modules].each do |ignored|
        assert_path_exists File.join(root, ignored, ".tamoz-vendored.tmp")
      end
    end
  end
  ...
  def test_a_vanishing_file_does_not_fail_construction
    with_workspace do |root|
      plant(root, ".tamoz-racing.tmp", mtime: ANCIENT)
      toolbox = Tamoz::Tools::Toolbox.new(root:, reap_staging: false, allow_changes: true)
      File.unlink(File.join(root, ".tamoz-racing.tmp"))
  ...
  def test_the_sweep_is_bounded
    with_workspace do |root|
      ...
      total.times { |index| plant(root, format(".tamoz-%04d.tmp", index), mtime: ANCIENT) }
  ...
  def test_a_swept_workspace_still_publishes_correctly
    with_workspace do |root|
      ...
      plant(root, ".tamoz-crashed.tmp", mtime: ANCIENT)

      toolbox = Tamoz::Tools::Toolbox.new(root:, allow_changes: true)

      assert_equal [".tamoz-crashed.tmp"], toolbox.reaped_staging
      toolbox.execute(
        "apply_patch",
        {"path" => "app.rb",
